...
import com.promineotech.art.service.user.ArtAddToOrderService;
...
public class DefaultArtAddToOrderController implements ArtAddToOrderController {
  ...
  public Order addToOrder(int art_id, int user_id, int order_id) {
    ...
    log.info(
        "An Order Request has been made to add art. Art: art_id = {} has been requested by user_id = {} for order = {}.",
        art_id, user_id, order_id);
    ...
    try {
      if (artAddToOrderService.checkStock(art_id)) {
        ...
        throw new ResponseStatusException(HttpStatus.NOT_FOUND,
            "Art is out of stock, try again later!");
      }
    } catch (TransientDataAccessResourceException e) {
      throw new ResponseStatusException(HttpStatus.NOT_FOUND,
          "The supplied art_id, user_id, or order_id does not exist.");
    }
